...
class SQLDatabase(Database):
    ...
    DETECTION_LOG_TABLENAME ="detection_log"
    ...
    CREATE_SONGS_TABLE = """
        ...
        Database.FIELD_EXTERNAL_ID,
        Database.FIELD_NUM_FRAMES,
        Database.FIELD_SONG_ID, Database.FIELD_SONG_ID, Database.FIELD_SONG_ID
    )

    CREATE_DETECTION_LOG_DATABASE = """
	CREATE TABLE IF NOT EXISTS `%s` (
	`%s` mediumint unsigned not null auto_increment,
	...
	DETECTION_LOG_TABLENAME,
	Database.FIELD_LOG_ENTRY_ID,
	Database.FIELD_TIMESTAMP,
	Database.FIELD_SONG_ID,
	Database.FIELD_SONGNAME,
	Database.FIELD_OFFSET,
	Database.FIELD_PROGRAM_NAME,
	Database.FIELD_EXTERNAL_ID,
	Database.FIELD_REGION,
	Database.FIELD_SOURCE_ID,
	Database.FIELD_PROGRAM_ID,
	Database.FIELD_SCHEDULE_ID,
	Database.FIELD_LOG_ENTRY_ID,
	Database.FIELD_LOG_ENTRY_ID,
	Database.FIELD_LOG_ENTRY_ID,
    )
    ...
    INSERT_SONG = "INSERT INTO %s (%s, %s, %s) values (%%s, UNHEX(%%s), %%s);" % (
        SONGS_TABLENAME, Database.FIELD_SONGNAME, Database.FIELD_FILE_SHA1, Database.FIELD_NUM_FRAMES)

    INSERT_IDENTIFICATION_LOG = "INSERT INTO %s (%s, %s, %s, %s, %s, %s, %s, %s, %s) values (%%s, %%s, %%s, %%s, %%s, %%s, %%s, %%s, %%s);" % (
	DETECTION_LOG_TABLENAME, 
        Database.FIELD_SONG_ID,
        Database.FIELD_SONGNAME,
        Database.FIELD_OFFSET,
    ...
    SELECT_SONG = """
        SELECT %s, HEX(%s) as %s, %s, %s FROM %s WHERE %s = %%s;
    """ % (Database.FIELD_SONGNAME, Database.FIELD_FILE_SHA1, Database.FIELD_FILE_SHA1, 
    Database.FIELD_NUM_FRAMES, Database.FIELD_EXTERNAL_ID,
    ...
    def setup(self):
        ...
        with self.cursor() as cur:
            ...
            cur.execute(self.CREATE_DETECTION_LOG_DATABASE)
    ...
    def insert_identification_log(self, song_id, song_name, offset, program_name, external_id, region, source_id, program_id, schedule_id):
        with self.cursor() as cur:
            cur.execute(self.INSERT_IDENTIFICATION_LOG, (song_id, song_name, offset, program_name, external_id, region, source_id, program_id, schedule_id))
	pass

    def insert_song(self, songname, file_hash, num_frames):
        """
        Inserts song in the database and returns the ID of the inserted record.
        """
        with self.cursor() as cur:
            cur.execute(self.INSERT_SONG, (songname, file_hash, num_frames))
